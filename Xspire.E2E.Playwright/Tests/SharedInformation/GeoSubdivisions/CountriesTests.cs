...
/// Trước mắt chỉ có 1 testcase success tạo mới Country.
...
public class CountriesTests : IClassFixture<TestBase>
{
    ...
    /// <summary>
    /// Edit Country vừa tạo: search theo Code, click vào Code để mở form Edit, sửa Description rồi lưu lại.
    /// Ensure edit thành công bằng cách quay về list, search lại và mở Edit lần nữa để kiểm tra Description mới.
    /// </summary>
    [Fact]
    public async Task TC_COUNTRY_EDIT_001_Should_Edit_Country_Description_Successfully()
    {
        ...
        // Tạm chờ 3s để bạn quan sát UI đã vào form Edit hay chưa (debug)
        ...
        // Đảm bảo đang ở detail với GUID thật, sau đó về lại list
        await countriesPage.EnsureSuccessOnDetailAsync();
        await countriesPage.NavigateBackToListAsync();

        // Search lại và kiểm tra Description đã được cập nhật trong grid (không cần mở Edit lần 2)
        await countriesPage.FillSearchAsync(CountriesTestData.SearchSuccess.Code);
        await countriesPage.EnsureSearchSuccessAsync(CountriesTestData.SearchSuccess.Code);
    ...
    /// <summary>
    /// Search theo mã sau khi đã tạo thành công (chạy sau TC_COUNTRY_NEW_001).
    /// Ensure search đúng: tạm thời chỉ assert ô search có giá trị đã nhập; bạn bổ sung assert kết quả lọc sau.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// Delete Country theo Code/Description:
    /// - Goto list Countries.
    /// - Search theo Code từ TestData.
    /// - Click vào ô Description để chọn dòng.
    /// - Mở menu Action (mũi tên dropdown) – bước mở menu delete.
    ///
    /// Phần ensure (xác nhận xoá thành công) sẽ được bàn thêm sau.
    /// </summary>
    [Fact]
    public async Task TC_COUNTRY_DELETE_001_Should_Open_Action_Menu_For_Selected_Row()
    {
        ...
        // Search lại và ensure mã CPC đã bị xoá (không còn bất kỳ row nào có Code = CPC).
        await countriesPage.FillSearchAsync(CountriesTestData.SearchSuccess.Code);
