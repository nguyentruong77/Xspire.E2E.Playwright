...
/// <summary>
/// Shared Information &gt; Geo Sales Structure &gt; Territory Level Definitions (list).
/// </summary>
public class TerritoryLevelDefinitionsPage
{
    private readonly IPage _page;
    private readonly PlaywrightSettings _settings;

    public TerritoryLevelDefinitionsPage(IPage page, PlaywrightSettings settings)
    {
        _page = page;
        _settings = settings;
    }

    public async Task EnsureOnTerritoryLevelDefinitionsListAsync()
    {
        var baseUrl = _settings.BaseUrl.TrimEnd('/');
        await _page.GotoAsync(baseUrl + "/SharedInformation/TerritoryLevelDefinitions");
        await _page.WaitForLoadStateAsync(LoadState.DOMContentLoaded);

        await Assertions.Expect(_page).ToHaveURLAsync(
            new System.Text.RegularExpressions.Regex(".*SharedInformation/TerritoryLevelDefinitions.*"),
    ...
    /// <summary>
    /// Detail URL must not be the all-zero new-record GUID.
    /// </summary>
