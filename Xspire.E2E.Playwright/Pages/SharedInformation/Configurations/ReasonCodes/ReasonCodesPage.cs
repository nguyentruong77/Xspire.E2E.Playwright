...
/// <summary>
/// Shared Information > Configurations > Reason Codes (list).
/// </summary>
public class ReasonCodesPage
{
    ...
    public ReasonCodesPage(IPage page, PlaywrightSettings settings)
    {
        _page = page;
        _settings = settings;
    }

    public async Task EnsureOnReasonCodesPageAsync()
    {
        await Assertions.Expect(_page).ToHaveURLAsync(new Regex(".*SharedInformation/ReasonCodes.*"));
    }

    public async Task EnsureOnReasonCodesListAsync()
    {
        var baseUrl = _settings.BaseUrl.TrimEnd('/');
        await _page.GotoAsync(baseUrl + "/SharedInformation/ReasonCodes");
        await _page.WaitForLoadStateAsync(LoadState.DOMContentLoaded);

        await Assertions.Expect(_page).ToHaveURLAsync(
            new Regex(".*SharedInformation/ReasonCodes.*"),
