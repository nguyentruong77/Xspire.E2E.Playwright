...
/// <summary>
/// Shared Information &gt; Configurations &gt; Attributes (list).
/// </summary>
public class AttributesPage
{
    ...
    public AttributesPage(IPage page, PlaywrightSettings settings)
    {
        _page = page;
        _settings = settings;
    }

    public async Task EnsureOnAttributesPageAsync()
    {
        await Assertions.Expect(_page).ToHaveURLAsync(new Regex(".*SharedInformation/Attributes.*"));
    }

    public async Task EnsureOnAttributesListAsync()
    {
        var baseUrl = _settings.BaseUrl.TrimEnd('/');
        await _page.GotoAsync(baseUrl + "/SharedInformation/Attributes");
        await _page.WaitForLoadStateAsync(LoadState.DOMContentLoaded);

        await Assertions.Expect(_page).ToHaveURLAsync(
            new Regex(".*SharedInformation/Attributes.*"),
