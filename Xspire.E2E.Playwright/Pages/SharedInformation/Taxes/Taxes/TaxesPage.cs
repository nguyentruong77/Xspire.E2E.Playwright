...
/// <summary>
/// Shared Information > Taxes > Taxes (list).
/// </summary>
public class TaxesPage
{
    ...
    public TaxesPage(IPage page, PlaywrightSettings settings)
    {
        _page = page;
        _settings = settings;
    }

    public async Task EnsureOnTaxesPageAsync()
    {
        await Assertions.Expect(_page).ToHaveURLAsync(new Regex(".*SharedInformation/Taxes.*"));
    }

    public async Task EnsureOnTaxesListAsync()
    {
        var baseUrl = _settings.BaseUrl.TrimEnd('/');
        await _page.GotoAsync(baseUrl + "/SharedInformation/Taxes");
        await _page.WaitForLoadStateAsync(LoadState.DOMContentLoaded);

        await Assertions.Expect(_page).ToHaveURLAsync(
            new Regex(".*SharedInformation/Taxes.*"),
